// locationScene.ts
// suimonAR (github.com/Haruto-Sa/suimonAR) の src/location/core.ts を再利用。
// LocAR + three.js による位置ベース AR の土台（カメラ・GPS平滑化・方位センサー・
// タッチfallback・高度処理）。実証済みのため原則そのまま使う。
// モデルの transform は呼び出し側（main.js）が設定ファイルから決める。
...
export class LocationScene {
  ...
  private webcamStatusCallbacks: Array<(status: { ok: boolean; code?: string; message?: string }) => void> = [];
  ...
  constructor(options: LocationSceneOptions = {}) {
    ...
    if (this.webcam.on) {
      this.webcam.on('webcamstarted', () => {
        ...
        for (const cb of this.webcamStatusCallbacks) cb({ ok: true });
      });
      this.webcam.on('webcamerror', (event: any) => {
        ...
        for (const cb of this.webcamStatusCallbacks) {
          cb({ ok: false, code: event?.code, message: event?.message });
        }
      });
    }
  ...
  /** カメラ起動の成否を受け取る（映像が出ない原因の可視化用）。 */
  onWebcamStatus(callback: (status: { ok: boolean; code?: string; message?: string }) => void): void {
    this.webcamStatusCallbacks.push(callback);
  }
  ...
  /** デバッグ描画（グリッド等）を追加するための three.js Scene。 */
