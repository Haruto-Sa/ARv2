// main.js — エントリポイント（LocAR + three.js）
//
// フロー:
//   1. 紹介ページ表示中に watergate-anchor.json を先読み（カメラはまだ起動しない）
...
//
// 設計方針（A-Frame 版から不変）:
//   - transform（位置/サイズ/回転/高さ）のベース値は watergate-anchor.json だけが供給する。
//   - カメラ追従・距離依存スケール・毎フレームの transform 書き換えはしない。
//   - GPS/方位の安定化は LocationScene が担う（モデルの scale/基準位置は変えない）。

import * as THREE from 'three';
import { GLTFLoader } from 'three/examples/jsm/loaders/GLTFLoader.js';
import { LocationScene } from './ar/locationScene';
import { loadAnchorConfig } from './ar/anchorConfig.js';
import { normalizeIntoOrigin } from './ar/normalizeModel.js';
import {
  ...
  renderInfoPanel,
  logConfigSnapshot,
} from './ar/debugHelpers.js';
import { createCorrectionUI } from './ar/correctionUI.js';

const ZERO_DELTA = { dx: 0, dz: 0, dy: 0, dYawDeg: 0, scaleMul: 1 };
...
async function boot() {
  try {
    const result = await loadAnchorConfig();
...
function setupAnimations(inner, clipsAll) {
  ...
  const clips = Array.isArray(cfg.clips)
    ? clipsAll.filter((c) => cfg.clips.includes(c.name))
    : clipsAll;
  ...
  console.info('[watergate-model] animations:', clips.map((c) => c.name));
