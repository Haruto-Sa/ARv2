...
//  - ここで触れるのは "補正デルタ" のみ。設定ファイル(watergate-anchor.json)の
...
const ZERO_DELTA = Object.freeze({ dx: 0, dz: 0, dy: 0, dYawDeg: 0, scaleMul: 1 });

export function loadCorrectionDelta() {
  try {
...
function saveCorrectionDelta(delta) {
  try {
...
// onChange(delta) は補正値が変わるたびに呼ばれる。
export function createCorrectionUI({ debug, onChange }) {
  ...
  // ステップ付きの ± ボタン行を作る。
  function row(label, key, step, isMul = false) {
  ...
  function apply() {
    ...
    if (typeof onChange === 'function') onChange({ ...delta });
  }
